...
const crearAccion = {
    agregarCancion : (cancion) => (
        function(dispatch){
            axios.post("http://localhost:3002/canciones",cancion)
            .then((resp) => {
                dispatch({
    ...
    eliminarCancion : (id) => (
        function(dispatch){
            axios.delete(`http://localhost:3002/canciones/${id}`)
            .then((resp) => {
                dispatch({
    ...
    modificarCancion : (cancion) => (dispatch) => {
        axios.put("http://localhost:3002/canciones/" + cancion.id, cancion)
        .then((resp) => {
            dispatch({
