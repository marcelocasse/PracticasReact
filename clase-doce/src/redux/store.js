...
const store = configureStore({
    reducer : (state,action) => {
        ...
        (action.type == ACT_ADD_CANCION) && (newState.canciones = [...state.canciones,action.data]);
        (action.type == ACT_ELIMINAR_CANCION) && (newState.canciones = state.canciones.filter((item) => (item.id !== action.data)));
...
axios.get("http://localhost:3002/canciones")
    .then((resp) => {
        store.dispatch({
            type : ACT_INIT_STORE,
            data : resp.data
